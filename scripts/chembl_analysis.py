...
if __name__ == "__main__":
    ...
    for model_type in ['gnn','mlp','bnn']:
        for acq_func in ['ucb', 'greedy', 'ei']:
            ...
            ### PLOTS FOR AUC
            fig, ax  = plt.subplots()
            sns.lineplot(plot_df, x="rogi", y="auc", hue="loss_type", marker='o', linestyle='', err_style='bars', hue_order=['mse', 'ranking', 'gp + mll'], alpha=0.95)
            ...
            ax.set_ylabel('AUC')
            ax.set_ylim([0, 0.35])        # will need to adjust based on your runs
            fig.savefig(f'rogi_auc_{model_type}_{acq_func}_nogp.png', bbox_inches='tight')
            plt.close()
            ...
            fig.savefig(f'rogi_max_frac_{model_type}_{acq_func}.png', bbox_inches='tight')
            plt.close()

            fig, ax  = plt.subplots()
            sns.lineplot(plot_df, x="rogi", y="max_frac", hue="loss_type", marker='o', linestyle='', err_style='bars', hue_order=['mse', 'ranking'], alpha=0.95)
            ax.get_legend().set_title("")
            ax.tick_params(axis='x', rotation=90)
            ax.set_xlabel('ROGI')
            ax.set_ylabel('Fraction of top 100 found')
            ax.set_ylim([0, 0.65])        # will need to adjust based on your runs
            fig.savefig(f'rogi_max_frac_{model_type}_{acq_func}_nogp.png', bbox_inches='tight')
            plt.close()

            fig, ax  = plt.subplots(figsize=(12, 6))
            sns.barplot(plot_df, x="dataset", y="max_frac", hue="loss_type", hue_order=['mse', 'ranking', 'gp + mll'], alpha=0.95, ax=ax)
            ax.get_legend().set_title("")
            ax.tick_params(axis='x', rotation=90)
            ax.set_xlabel(None)
            ax.set_ylabel('Fraction of top 100 found')
    ...
    all_results = all_results.drop_duplicates(['dataset', 'loss', 'acq_func', 'model_type']) # gps will be duplicated for plots
        ...
        for af in ['ei', 'greedy', 'ucb']:
            ...
            for stats in statistics:
                if mt != 'gp':
                    ...
                    tmp[f'{stats}_mse'] = '$' + tmp[f'{stats}_mean']['mse'].apply(lambda x:f'{x:.4f}') + ' \pm ' + tmp[f'{stats}_95ci']['mse'].apply(lambda x:f'{x:.4f}') + '$'
                    tmp[f'{stats}_ranking'] = '$' + tmp[f'{stats}_mean']['ranking'].apply(lambda x:f'{x:.4f}') + ' \pm ' + tmp[f'{stats}_95ci']['ranking'].apply(lambda x:f'{x:.4f}') + '$'
                    ...
                    tmp[stats] = '$' + tmp[f'{stats}_mean']['gp + mll'].apply(lambda x:f'{x:.4f}') + ' \pm ' + tmp[f'{stats}_95ci']['gp + mll'].apply(lambda x:f'{x:.4f}') + '$'
                    keep_cols += [stats]
