...
if __name__ == "__main__":
        ...
        for acq_func in ["ucb", "greedy", "ei"]:
            ...
            for corr in ["spearman", "kendall", "r2", "ndcg"]:
                ...
                for k, loss_type in enumerate(["mse", "ranking", "gp + mll"]):
                    subdf = plot_pred[plot_pred["loss_type"] == loss_type].sort_values(
                        "max_frac_mean"
                    )
                    try:
                        plt.errorbar(
                            subdf["max_frac_mean"],
                            subdf[f"{corr}_mean"],
                            xerr=subdf["max_frac_95ci"],
                            yerr=subdf[f"{corr}_95ci"],
                            fmt="none",
                            alpha=0.3,
                            color=palette[k],
                        )
                        ...
                        plt.plot(
                            subdf["max_frac_mean"], lin_fit, color=palette[k], alpha=0.7
                        )
                        plt.ylim([-0.1, 1])
                        p_rho, p_p = pearsonr(
                ...
                # plt.ylim([0, 0.25])        # will need to adjust based on your runs
                plt.savefig(
                    f"perf_v_opt_{corr}_{model_type}_{acq_func}.png",
                    bbox_inches="tight",
                )
                plt.close()
                ...
                # plt.ylim([0, 0.25])        # will need to adjust based on your runs
                plt.savefig(
                    f"perf_v_rogi_{corr}_{model_type}_{acq_func}.png",
                    bbox_inches="tight",
                )
                plt.close()
    ...
    for mt in ["gnn", "mlp", "bnn", "gp"]:
        for af in ["ei", "greedy", "ucb"]:
            ...
            tmp = pd.pivot(
                tmp, values=cols, index=["correlation"], columns=["loss_type"]
            )
            ...
            tmp.to_latex(
                f"latex_table_{af}_{mt}_perf_v_opt.txt",
    ...
    all_results = all_results.drop_duplicates(
        ["dataset", "loss_type", "acq_func", "model_type"]
    )  # gps will be duplicated for plots
